...
def call_result_text(result) -> str:
    ...
    print(f"\n--- MCP response (is_error={result.is_error}) ---\n{text}\n--------------------")
    return text
...
def call_result_json(result) -> dict:
    ...
    (`is_error=True`), not a `{}`/text success — assert on `is_error`, not on
    the text prefix, so a silently-swallowed failure can never slip through.
    """
    text = call_result_text(result)
    assert not result.is_error, f"Tool returned an error: {text}"
    return json.loads(text)


def assert_tool_error(result) -> str:
    """Assert a CallToolResult is a real MCP tool error (is_error=True).

    v1.8.0: upstream 4xx/5xx and local validation failures must surface as
    is_error=True, carrying the upstream status code and message — never a
    silently-returned `{}` or text success. Returns the message text for
    additional diagnostics (e.g. substring checks).
    """
    text = call_result_text(result)
    assert result.is_error, f"Expected an MCP tool error (is_error=True), got a success result: {text}"
    return text
